...
/** How long a signed intro-preview URL stays valid. */
 ...
 *
 * This exists so a paid scene that has no uploaded trailer still HAS an intro
 * instead of an empty black box. The creator-supplied trailer
 * (`teaserBunnyVideoId`) stays the preferred asset because somebody chose it;
 * this is the floor beneath it, generated by Bunny from frames it already
 * holds.
 *
 * Safe to hand to a non-buyer for two reasons, and both matter:
 *
 *   1. It is a SEPARATE asset. No segment of the scene is ever signed, so the
 *      token cannot be repurposed into playback however the clip is used — the
 *      same reason a `teaserBunnyVideoId` is safe and signing `playlist.m3u8`
 *      is not.
 *   2. The token is signed for the FILE, not the folder. Bunny opens every path
 *      under a folder it signed, so signing `/{id}/` would also open
 *      `/{id}/playlist.m3u8`; signing `/{id}/preview.webp` opens that one URL
 *      and nothing else. Asserted in tests/bunny.test.ts.
 *
 * Null when the library is not configured or the row is not Bunny-hosted (a
 * demo or side-loaded row has no such asset). Bunny only generates the
 * animation when the library's previews are enabled, so a 404 is possible and
 * the caller must treat that as "no intro", not as a broken page.
 */
/**
 * The in-app URL the client should use for the intro animation:
 * `/api/videos/<rowId>/intro`.
 *
 * The browser must NOT fetch the CDN URL directly, and this is not a stylistic
 * choice: this pull zone has hotlink protection on, so it answers **403 to any
 * request carrying a `Referer`** and 200 to the same URL without one. A browser
 * always sends a `Referer` for an <img>, so the signed URL that works perfectly
 * in curl renders as a broken image in the page. Measured, not assumed.
 *
 * Proxying also keeps the row's Bunny GUID on the server, where every other part
 * of the playback design keeps it.
 *
 * Null when the row cannot be served this way — no row id, no Bunny id, a
 * fabricated `demo-` id, or an unconfigured library.
 */
export function introPreviewPath(video: VideoSourceLocation): string | null {
  if (!video.id || !video.bunnyVideoId) return null;
  if (!isBunnyPlaybackConfigured()) return null;
  if (!isBunnyVideoId(video.bunnyVideoId)) return null;
  return `/api/videos/${encodeURIComponent(video.id)}/intro`;
}

/**
 * The signed CDN URL for the animation. Server-side only — see
 * introPreviewPath for why the browser must never be given this directly.
 */
export function resolveIntroPreviewUrl(
  video: VideoSourceLocation,
  expirationMinutes = INTRO_PREVIEW_MINUTES
): string | null {
  if (!video.bunnyVideoId) return null;
  if (!isBunnyPlaybackConfigured()) return null;

  const path = `/${video.bunnyVideoId}/preview.webp`;
  const expiresAt = Math.floor(Date.now() / 1000) + expirationMinutes * 60;
